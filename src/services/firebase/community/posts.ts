...
import { collection, orderBy, query, onSnapshot, QuerySnapshot, FirestoreError, getFirestore } from 'firebase/firestore';
...
const analyticsLogosHttpsCallable = httpsCallable(functions, 'analyticsLogos', {});

export const getPosts = async ({
    searchTerm,
    modules = ["posts"],
}: { searchTerm: string, modules?: ("posts")[] }) => {
...
export const userPic = (user: any) => {
    if (!user?.PictureDate || !user?.Id) return "https://static.vecteezy.com/system/resources/thumbnails/003/337/584/small/default-avatar-photo-placeholder-profile-icon-vector.jpg"
...
export async function fetchPendingMarketItemLogos(marketItems: any, setLogo: any) {
