...
export const TextInput: FC<{ label: string, type: string, name: string, placeholder?: string | null, disabled?: boolean, labelClassName?: string, className?: string }> = ({ label, labelClassName, ...props }) => (
    <Col className="gap-4">
        ...
        <Field max={new Date().toISOString().split('T')[0]} className={props.className ? props.className : "text-base rounded-lg text-start block w-full overflow-auto p-2.5 bg-grey-3 placeholder-grey-1 h-[48px] justify-center text-white border-none"} {...props} />
    </Col>
);
