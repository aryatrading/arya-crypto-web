...
import {
  converterTop6Coins,
  usdt,
} from "../../../utils/constants/defaultConverterList";
...
const inputClasses =
  "font-bold text-white bg-transparent flex-1 h-[50px] pl-4 mr-12 border-transparent focus:ring-0 focus:border-0 focus:outline-none";
...
const numericInput = (
  ...
  return (
    <input
      className={clsx({ "cursor-not-allowed": disabled }, inputClasses)}
      type="number"
      value={value}
      placeholder="0"
      onChange={onChange}
...
export const CoinConverter = (props: CoinConverterTypes) => {
  ...
  const [secondCoin, setSecondCoin] = useState<AssetType>({
    ...usdt,
    currentPrice: _assetprice[usdt.symbol] || 1,
  });
  ...
  const convertValues = useCallback(
    (amount: number, currentPrice1: number, currentPrice2: number) => {
      return amount * (currentPrice1 / currentPrice2);
    },
    []
  );

  const onChnageFirstCoinAmount = useCallback(
    (event: any) => {
      ...
      if (value === "") {
        ...
        if (firstCoin?.name === secondCoin?.name) {
          ...
          setSecondCoinAmount(
            convertValues(
              parseFloat(value || 0),
              firstCoin?.currentPrice || 0,
              secondCoin?.currentPrice || 0
            )
          );
        }
      }
  ...
  const onChnageSecondCoinAmount = useCallback(
    (event: any) => {
      const value = event.target.value;
      setSecondCoinAmount(value);
      if (value === "") {
  ...
  const defaultList = useMemo(
    () =>
      converterTop6Coins.map((e) => {
        const second = { ...usdt, currentPrice: _assetprice[usdt.symbol] };
        const onClick = () => {
          if (!props.preDefined) {
            const currentPrice = _assetprice[e.symbol];
            ...
            if (e?.name === firstCoin?.name) {
              ...
              setSecondCoinAmount(
                convertValues(
                  parseFloat(firstCoinAmount.toString()),
                  firstCoin?.currentPrice || 0,
                  currentPrice || 0
                )
              );
        ...
        return (
          <Button
            key={e.symbol}
            className="bg-grey-3 rounded-md text-grey-1 hover:text-white hover:bg-grey-4 w-full px-5 py-2.5 lg:py-0 lg:px-2 xl:font-sm  font-xs font-bold h-[50px]"
            onClick={onClick}
          >
            {props.preDefined
              ...
              : e?.symbol.toUpperCase() + " > " + usdt?.symbol.toUpperCase()}
          </Button>
        );
      }),
    [_assetprice, convertValues, firstCoin, firstCoinAmount, props.preDefined]
  );
    ...
    >
      ...
      <>
        <Row
          className={
            props.isFullPage
              ? "flex-col-reverse gap-6 bg-transparent rounded-md items-center justify-center w-full"
              : "flex-col gap-6 bg-transparent rounded-md lg:flex-row items-start justify-start w-full"
          }
        >
          <Col className="justify-center gap-6 w-full lg:w-3/5">
            ...
            <Row className="gap-4 items-center bg-grey-3 px-4 rounded-md">
              ...
              {numericInput(
                firstCoinAmount,
                onChnageFirstCoinAmount,
                !firstCoin?.name || !secondCoin?.name
              )}
            </Row>
            <Row className="gap-4 items-center bg-grey-3 px-4 rounded-md">
              <AssetDropdown
              ...
              {numericInput(
                secondCoinAmount,
                onChnageSecondCoinAmount,
                !secondCoin?.name || !secondCoin?.name
              )}
            </Row>
          </Col>
          <div
            className={
              props.isFullPage
                ? "flex flex-row gap-4 w-full justify-items-start h-full mb-4"
                : "grid grid-cols-2 md:grid-cols-3 gap-6 w-full justify-items-start lg:w-2/5 h-full"
            }
          >
            {defaultList}
          </div>
