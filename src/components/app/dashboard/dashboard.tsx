...
const Dashboard: FC = () => {
  ...
  const [portfolioHoldings, setPortfolioHoldings] = useState<
    PortfolioAssetType[]
  >([]);
  const [totalPortfolioAmountInUSD, setTotalPortfolioAmountInUSD] =
    useState<number>(0);
    ...
    getPortfolioHoldings(selectedExchange?.provider_id)
      .then((res) => {
        ...
        if (selectedExchange?.provider_id) {
          const assets: PortfolioAssetType[] =
            data[selectedExchange?.provider_id]?.data;
          if (assets?.length) {
  ...
  const portfolioLineChart = useMemo(() => {
    ...
    const smartAllocationData: chartDataType[] = portfolioSnapshots?.map(
      (snapshot) => {
        const time = new Date(snapshot.created_at).getTime();
        return {
          time: Math.floor(time / 1000) as chartDataType["time"],
    ...
    return (
      <LineChart
        ...
        secondaryLineData={smartAllocationData}
        className={clsx("sm:col-span-2 col-span-3 h-[400px]", {
          "sm:col-span-3": !selectedExchange?.provider_id,
        })}
      />
    );
            ...
            {!portfolioHoldings.length ? (
              ...
              portfolioHoldings.map((asset) => {
                const isPriceChangePositive =
                  asset.asset_details.price_change_percentage_24h > 0;
                ...
                const assetPortfolioPercentage =
                  (asset.current_value / totalPortfolioAmountInUSD) * 100;
                  ...
                  <tr key={asset.name}>
                    ...
                    <td>
                      <Row className="justify-between items-center w-[120px]">
                        <p>
                          {percentageFormat(
                            (asset.current_value / totalPortfolioAmountInUSD) *
                              100
                          )}
                          %
                        </p>
                    ...
                    >
                      {formattedChangePercentage}% ({formattedChangePrice})
                    </td>
                    <td>
                      <Row className="gap-2">{tableExchangesImages}</Row>
                    </td>
                  </tr>
                );
              })
  ...
  return (
    <Col className="w-full grid grid-cols-12 md:gap-10 lg:gap-16 pb-20 items-start justify-start">
      ...
      {(isLoadingPortfolioSnapshots ||
        ...
        exchangeStoreStatus === StatusAsync.PENDING) &&
        loadingOverlay}
    </Col>
  );
};
