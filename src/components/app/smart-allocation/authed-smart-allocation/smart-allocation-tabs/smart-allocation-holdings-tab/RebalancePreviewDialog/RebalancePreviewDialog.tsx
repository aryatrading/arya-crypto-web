            ...
            <CustomScroll height='h-[400px] md:h-[250px]'>
                {
                    holdingData.map((asset) => {
                        const {asset_details, current_weight, weight, current_value, expected_value} = asset
                        const asset_data = asset_details?.asset_data

                        let percentage = 0
                        let buy = false
                        if(weight&&current_weight){
                            const currentWeight = current_weight * 100
                            const targetWeight = weight * 100
                            const diff = currentWeight - targetWeight

                            if(diff < 0){
                                buy = true
                                percentage = Math.abs(Math.round((Math.round(diff) / targetWeight) * 100))
                            }
                            else{
                                percentage = Math.abs(Math.round((Math.round(diff) / currentWeight) * 100))
                            }
                        }
                        return <div className='grid grid-cols-5 rounded-lg items-stretch justify-between text-center font-medium md:font-semibold'>
                                    ...
                                    <div className='col-span-2 py-4 px-4 flex justify-center'>
                                        ...
                                        {buy&&<div style={{width:`${percentage}%`}} className={twMerge(' absolute right-0 h-full rounded bg-red-1')}></div>}
                                        </div>
                                        <div className='w-1/2 relative'>
                                        {!buy&&<div style={{width:`${percentage}%`}} className={twMerge(' absolute left-0 h-full rounded bg-green-1')}></div>}
                                        </div>
                                    </div>
                                    <div className='col-span-1 py-4 px-4 text-sm md:text-base'>
