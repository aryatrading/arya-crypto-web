...
const displayClasses =
  "text-base rounded-md block w-full overflow-auto p-2.5 bg-grey-3 placeholder-grey-1 h-[48px] justify-center text-white";

const ExchangeAccordionCard = ({
  cardId,
  exchange,
  setExchanges,
  exchanges,
  ...
  const name = useMemo(
    () =>
      exchange?.create
        ...
        : exchange.name,
    [exchange?.create, exchange.name]
  );
  const exchangeDetails =
    getRemoteConfigValue("exchanges")?.[exchange.provider_id];

  const openModal = useCallback(
    (type: string) => setShowModal({ bool: true, type }),
    []
  );
  const closeModal = useCallback(
    () => setShowModal({ bool: false, type: "" }),
    []
  );
  ...
  const copyWhiteListIPs = useMemo(
    () => (
      <Row className="items-start gap-4 h-[90px] flex-col lg:flex-row lg:items-center">
        ...
        <Button
          className="text-white bg-grey-3 hover:bg-grey-4 px-4 font-medium rounded-md text-base min-h-[44px] mt-6"
          type="button"
          onClick={() => {
        ...
        {({ isSubmitting }) => (
          <Form className="flex flex-col w-full gap-4">
            ...
            <Row className="gap-4">
              <Button
                className={
                  "text-white mt-14 lg:mt-0 font-medium rounded-md text-base min-h-[44px] focus:outline-none self-start px-10 bg-blue-1 hover:bg-blue-2"
                }
                type="submit"
              ...
              <Button
                className={
                  "text-white mt-14 lg:mt-0 font-medium rounded-md text-base min-h-[44px] focus:outline-none self-start px-10 bg-transparent"
                }
                type="button"
                disabled={isSubmitting}
                onClick={() => {
                  setExchanges((st: ExchangeType[]) =>
                    st.filter((e) => !e?.create)
                  );
                }}
              >
                {t("cancel")}
  ...
  const display = useMemo(() => {
    ...
    return (
      <Col className="flex flex-col w-full gap-4">
        <Row className="gap-4 flex-col lg:flex-row">
          <Col className="flex-1 gap-4">
            ...
            <Col className="relative w-full overflow-hidden">
              <Button
                className="text-base rounded-md text-start block w-full overflow-auto lg:w-full p-2.5 bg-grey-3 placeholder-grey-1 h-[48px] justify-center text-white pr-4"
                onClick={edit}
              >
                {exchange?.name || ""}
              </Button>
          ...
          <Col className="flex-1 gap-4">
            ...
            <Col className={displayClasses}>
              {new Date(exchange?.creation_date || "").toLocaleDateString()}
            </Col>
          </Col>
        </Row>
        <Col className="gap-2">
        ...
        <Button
          className={
            "text-red-1 mt-20 lg:mt-0 font-medium rounded-md text-base min-h-[44px] focus:outline-none self-start px-10 bg-red-2 hover:divide-opacity-95"
          }
          onClick={toggleModal}
        >
          ...
          className={clsx(
            {
              "opacity-50 cursor-not-allowed":
                exchanges.filter((e: ExchangeType) => e?.create).length > 0 &&
                !exchange.create,
            },
            "flex-row w-full focus:outline-none focus:ring-0 focus:border-0",
            className,
  ...
  const AccordionItem = useMemo(
    () => (
      <Accordion.Item
        className={"w-full bg-grey-2 mb-6 rounded-md"}
        value={cardId}
      >
        <AccordionTrigger>
        ...
        <AccordionContent>
          <Row className="flex-col-reverse gap-10 lg:flex-row lg:gap-5 p-6">
            <Col className="flex-1">{exchange.create ? form : display}</Col>
            <Col className="flex-[1] gap-6">
              <Row className="justify-between items-center bg-yellow-1 rounded-md flex-col md:flex-row">
                ...
                <Button
                  className={clsx(
                    {
                      "w-auto": i18n.language === "en",
                      "w-[300px]": i18n.language === "fr",
                    },
                    "text-white font-medium min-w-[150px] rounded-md text-base focus:outline-none bg-grey-2 hover:divide-opacity-95 h-[44px] self-center me-0 mb-4 md:mb-0 md:me-6"
                  )}
                  onClick={() =>
                    window.open(exchangeDetails?.exchangeURL, "_blank")
                  }
                >
              ...
              <Row className="justify-between items-center bg-blue-3 rounded-md h-[65px] px-6">
                <p className="text-white font-bold text-lg">
                  {t("howToConnect", { name: name })}
                </p>
                <h5
                  className="text-blue-1 font-bold cursor-pointer"
              ...
              <Col className="gap-2">
                {exchangeDetails?.steps?.[i18n?.language]?.map(
  ...
  return (
    <Row className="gap-10 w-full overflow-hidden">
      {AccordionItem}
      <Modal
        isVisible={showModal.bool}
        size={showModal.type === "video" ? "4xl" : "lg"}
      >
        <ExchangeModals
          closeModal={closeModal}
          showModal={showModal}
          id={exchange.provider_id}
          name={name}
        />
      </Modal>
    </Row>
