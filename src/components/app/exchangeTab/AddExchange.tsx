...
const AddExchange = (props: AddExchangeTypes) => {
    ...
    const gridView = useMemo(() => {
        ...
        return (
            <Col className={clsx({ "grid-cols-3": isOdd && !isTabletOrMobileScreen, "grid-cols-2": !isOdd && !isTabletOrMobileScreen, "grid-cols-1": isTabletOrMobileScreen }, "grid gap-4")}>
                {
                    allProviders?.map(provider => {
                        return (
                            <Button key={provider.id} className={clsx({ "hover:bg-grey-4": !provider?.isConnected, "cursor-not-allowed": provider?.isConnected }, 'w-[214px] h-[48px] bg-grey-3 rounded-md')} onClick={() => props.onPressExchange((st: any) => [...st, { provider_id: provider.id, name: provider.name, create: true }])} disabled={provider?.isConnected}>
