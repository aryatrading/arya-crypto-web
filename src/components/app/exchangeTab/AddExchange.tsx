...
const AddExchange = (props: AddExchangeTypes) => {
  ...
  const gridView = useMemo(() => {
    ...
    return (
      <Col
        className={clsx(
          {
            "grid-cols-3": isOdd && !isTabletOrMobileScreen,
            "grid-cols-2": !isOdd && !isTabletOrMobileScreen,
            "grid-cols-1": isTabletOrMobileScreen,
          },
          "grid gap-4"
        )}
      >
        {allProviders?.map((provider) => {
          return (
            <Button
              ...
              className={clsx(
                {
                  "hover:bg-grey-4": !provider?.isConnected,
                  "cursor-not-allowed": provider?.isConnected,
                },
                "w-[214px] h-[48px] bg-grey-3 rounded-md"
              )}
              onClick={() =>
                props.onPressExchange((st: any) => [
                  ...st,
                  {
            ...
            >
              <Row className="px-4 justify-between items-center">
                <Row className="justify-center items-center gap-2">
                  ...
                  <label className="font-bold text-base text-center text-white">
                    {provider.name.charAt(0) +
                      provider.name.slice(1).toLowerCase()}
                  </label>
                </Row>
                {provider?.isConnected && (
                  <CheckBadgeIcon stroke={"#22C55E"} className="w-6 h-6" />
                )}
              </Row>
            </Button>
          );
  ...
  return (
    <Col className={classes}>
      ...
      <label className="font-medium text-base text-center text-grey-1">
        {t("exchangeHint").split(".")[0]}.<br />
        {t("exchangeHint").split(".")[1]}
      </label>
      {gridView}

      <label className="font-bold text-lg text-center text-white">
        {t("exchangeFutureInfo")}
      </label>
    </Col>
  );
};
